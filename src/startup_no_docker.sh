...
# 5. Pasar token.json desde GCS a la VM
...
# 6. Ejecutar el script directamente con logging
echo "Ejecutando script: $SCRIPT"
python3 "/root/purkinje-learning/$SCRIPT" &> "/root/output/$OUT_NAME"
SCRIPT_EXIT_CODE=$?

# 7. Verificar exit code y subir resultado a GCS
if [ "$SCRIPT_EXIT_CODE" -eq 0 ]; then
    ...
    echo "ERROR: La ejecución del script falló con código $SCRIPT_EXIT_CODE." >&2

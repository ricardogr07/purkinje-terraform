...
def main(gpu_config, wait=False):
    ...
    gpu_types = gpu_config["instance_config"]["gpu_type"]
    if isinstance(gpu_types, str):
        gpu_types = [gpu_types]

    machine_types = gpu_config["instance_config"].get("machine_type", [])
    if isinstance(machine_types, str):
        machine_types = [machine_types]

    for machine_type in machine_types:
        for gpu_type in gpu_types:
            print(f"\nTrying GPU type: {gpu_type} with machine type: {machine_type}")
            config_copy = gpu_config.copy()
            config_copy["instance_config"] = config_copy["instance_config"].copy()
            config_copy["instance_config"]["gpu_type"] = gpu_type
            config_copy["instance_config"]["machine_type"] = machine_type
            try:
                available_zones = check_machine_type_and_accelerator(
                    compute,
                    config_copy["project_id"],
                    machine_type,
                    gpu_type,
                    compute_zones
                )
                ...
                accelerators = get_accelerator_quota(
                    ...
                    config_copy,
                    available_zones,
                    config_copy["instance_config"]["number_of_gpus"]
                )

                available_regions = list({v['region'] for v in available_zones})

                if available_regions:
                    print(f"Machine type {machine_type} with GPU {gpu_type} is available in: {available_regions}")
                    instance_details = create_instance(compute, config_copy["project_id"], config_copy, accelerators)

                    if len(instance_details) > 0:
                        print(f"Created instances: {instance_details}")
                        if wait:
                            print("Hit enter to delete instances")
                            input()
                        delete_instance(compute, config_copy["project_id"], instance_details)
                        return
                else:
                    print(f"No regions found for machine type {machine_type} with GPU {gpu_type}")
            except Exception as e:
                print(f"Skipping GPU type {gpu_type} due to error: {e}")
